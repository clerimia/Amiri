...
| 切分器 | `MarkdownHeaderTextSplitter`（切到 `###`，metadata 带完整标题路径）-> `RecursiveCharacterTextSplitter` | 两段式 |
| 批注预处理 | `> 【clerimia：】 xxx` -> `**[clerimia 注]** xxx`，splitter 前 | (β) 标记保留 |
| 切分参数 | `chunkSize=1000, chunkOverlap=200`（字符） | 后续 benchmark 调参 |
| embedding | Ark 豆包 `doubao-embedding-vision`，1024 维 | 修正 #3 决策 5c：非 OpenAI 直连，走 Ark OpenAI 兼容接口 |
| 检索方式 | 混合检索（向量 + BM25 + RRF 融合） | langchain `EnsembleRetriever`，客户端融合 |
| BM25 | langchain `BM25Retriever`（内存），配中文分词 | 非服务端混合（m1，非 m2） |
| rerank | 不做 | 档位 1 体量小，无收益 |
| tool 返回 | 结构化 markdown 给 LLM + `details` 旁路 JSON 给前端 | f2-B，f1 增强版 |
| tool 调用 | agent 自主调（t1，Agentic RAG） | 非"每次先检索" |
| tool 命名 | `search_blog` / `search_interview` | 是否合并存疑，见地图迷雾 |
| 两个 tool 差异 | pipeline 实现共享；差异仅在素材源/collection/tool 名/system prompt | 检索参数暂不差异化 |
...
- **第一段**：`MarkdownHeaderTextSplitter`，切到 `###` 级，metadata 带完整标题路径（如 `["## 1. AgentLoop", "### 核心循环引擎 runLoop()"]`）
...
- collection 维度 = 1024（豆包 embedding）
...
- Ark 豆包 `doubao-embedding-vision`，1024 维，volcengine provider
- 接口走 OpenAI 兼容（`api_base` + `api_key`），用 langchain.ts 的 OpenAI embedding 适配器
- **修正 #3 决策 5c**：原说"OpenAI text-embedding-3-small（1536 维）"，实际是 Ark 平台的豆包 embedding（1024 维）。架构不变（langchain OpenAI 适配器照常用，`baseURL` 指向 Ark），仅维度和模型名变

### 2.2 混合检索（B2）

- **主决策**：混合检索（向量 + BM25），非纯向量
- **实现**：langchain `EnsembleRetriever` 组合：
  - `QdrantVectorStore.asRetriever()`（向量侧）
  - `BM25Retriever`（关键词侧，内存版，配中文分词如 `nodejieba`）
- **融合**：RRF（Reciprocal Rank Fusion），langchain `EnsembleRetriever` 内置支持
- **非 Qdrant 服务端混合（m2）的原因**：Ark 只提供 dense embedding，sparse 侧要自己造；客户端 RRF（m1）可见、可讲、和 Ark dense-only 配置契合
...
- 混合检索召回候选数 N = 20（向量 + BM25 各 20，喂给 RRF）
- 最终 top-k = 5（RRF 融合后取前 5 喂给 agent）
...
- OpenAI embedding 适配器（指向 Ark）
- `QdrantVectorStore`（向量存储 + retriever）
- `BM25Retriever`
- `EnsembleRetriever`
...
5. **自己训 embedding**--用 Ark 豆包
6. **非 Qdrant store**（`InMemoryStore`/`docstore`）--vector store 只 Qdrant
7. **Output parser / 结构化输出**--LLM 输出直接是文本，Pi agent 自己组织
8. **Document Transformer 现成组件**（`LongContextReorder`/`EmbeddingsFilter`）--和 B3 不做 rerank 一致
9. **Callbacks / Tracing**--可观测走 Pi event + 自己日志
10. **Prompt templates**--agent prompt 由 Pi session 管

### 3.3 薄封装点（自己写一层，落 `.pi/extensions/lib/`）

- (D-1) **批注预处理函数** `preprocessAnnotations(md)`：splitter 前
- (D-2) **BM25 中文分词配置**：`BM25Retriever` 构造时传分词函数
- (D-3) **检索结果 -> tool 返回值转换**：`Document[]` -> `{markdown, details}`，在 tool execute 里
...
.pi/extensions/
  lib/
    rag-ingest.ts      # loader + preprocessAnnotations + splitter
    rag-retrieve.ts    # retriever 初始化 + 结果转换 + 分词配置
  blog-rag.ts          # search_blog tool（import lib）
  interview-rag.ts     # search_interview tool（import lib）
scripts/
...
- 素材源（见 1.2）
- collection 名（各自一个）
- tool 名（`search_blog` / `search_interview`）
...
- 为什么用 `MarkdownHeaderTextSplitter` + `RecursiveCharacterTextSplitter` 两段式，不只用 recursive
...
- **BM25 公式项含义**（TF 项、IDF 项 `log((N-n+0.5)/(n+0.5))` 的 +0.5 是 Lidstone 平滑、文档长度归一化项）-- **L3 核心**
- 为什么中文 BM25 要分词
...
- **后续 RAG 实现 ticket**：`.pi/extensions/blog-rag.ts` / `interview-rag.ts` + `lib/`，按本文档拆摄入/检索/collection schema
- **后续环境变量 ticket**：`.env.example` 加 `BLOG_POSTS_DIR`（+ 具体 env 变量清单 + zod 校验）
- **后续中文分词 ticket**：`BM25Retriever` 配 `nodejieba`，处理中文分词
- **docker-compose.yml**：开发态 Qdrant 编排（#3 决策 5a 已定，进 git）

## 7. 尚未明确（进地图）

- **benchmark 调参**：chunk size/overlap、top-k/N、RRF k 等，RAG 跑通后做 benchmark 优化
- **是否合并两个 tool**：`search_blog`/`search_interview` 是否合并为一个参数化 `search(collection)` tool，实际使用后评估
